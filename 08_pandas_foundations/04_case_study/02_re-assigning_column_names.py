...
pandas has been imported in the workspace as pd, and the file NOAA_QCLCD_2011_hourly_13904.txt has been parsed and
loaded into a DataFrame df. The comma separated string of column names, column_labels, and list of columns to drop,
list_to_drop, have also been loaded for you.

Instructions:
*   Convert the comma separated string column_labels to a list of strings using .split(','). Assign the result to
    column_labels_list.
...
df = pd.read_csv('../_datasets/NOAA_QCLCD_2011_hourly_13904.txt')
...
df_dropped.to_csv('../_datasets/NOAA_QCLCD_2011_dropped.txt', index=False)
